...
public class PanoramicProductMaterialsServiceImpl extends AbstractService<PanoramicProductMaterials>
		implements PanoramicProductMaterialsService {
	private static final transient TLogger DB_LOGGER = LoggerUtils
			.getLogger(PanoramicProductMaterialsServiceImpl.class);
	@Autowired
	...
	public List<PanoramicProductMaterials> listRealTimeProductSummaryCategoryTask() {
		List<PanoramicProductMaterials> productMaterials = productMaterialsMapper
				.listRealTimeProductSummaryCategoryTask();
	...
	public void productSummaryTask(String name, String code, String date) {
		...
		condition.createCriteria()
				.andCondition("  substring(code, 1, 12) = '" + code + "' AND f_id=2 AND delete_flag=1 "
	...
	public Double summaryByCodeAndDate(String code, String date) {
		double inSum = productMaterialsMapper.summaryByCodeAndDate(code, date, 1) == null ? 0
				: productMaterialsMapper.summaryByCodeAndDate(code, date, 1);
		double outSum = productMaterialsMapper.summaryByCodeAndDate(code, date, 0) == null ? 0
				: productMaterialsMapper.summaryByCodeAndDate(code, date, 0);
		return inSum - outSum;
	}
}
