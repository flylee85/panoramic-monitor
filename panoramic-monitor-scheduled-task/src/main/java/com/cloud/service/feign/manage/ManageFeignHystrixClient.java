 ...
 * @author summer
 */
@FeignClient(name = "monitor-manage", configuration = FeignConfiguration.class, fallback = ManageFeignHystrixClient.HystrixClientFallback.class)
@Service
public interface ManageFeignHystrixClient {
     ...
     * 产品实时汇总
     */
    @RequestLine("POST /product/materials/task")
    void realTimeProductSummaryTask();

    /**
     * 实时消耗数据汇总
     */
    @RequestLine("POST /real/time/consumption/task")
    void realTimeConsumptionSummaryTask();

    /**
     * 异常信息状态定时刷新
     */
    @RequestLine("POST /panoramic/inventory/entry/task")
    void manualEntryExceptionRecordTask();

    /**
     * 每日库存定时汇总
     */
    @RequestLine("POST /daily/inventory/summary/task")
    void dailyInventorySummaryTask();

    /**
     * 异常出库信息状态定时刷新
     */
    @RequestLine("POST /into/the/factory/records/task")
